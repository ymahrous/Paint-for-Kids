...
ChangeFclr::ChangeFclr(ApplicationManager* pApp, CFigure* Selected) :Action(pApp), Fig(Selected){}

void ChangeFclr::ReadActionParameters() {
...
void ChangeFclr::Execute() {
	ReadActionParameters();
	Input* pIn = pManager->GetInput();
	Output* pOut = pManager->GetOutput();
	if (pManager->SelFigCount() == 1) { //selected figure count =1
		Fig->ChngFillClr(c);
		pOut->PrintMessage("Filling color changed!");
	}
